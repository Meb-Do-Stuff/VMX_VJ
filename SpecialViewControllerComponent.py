...
class DetailViewControllerComponent(ControlSurfaceComponent):
    ...
    def set_device_clip_toggle_button(self, button: ButtonElement):
    ...
    def set_detail_toggle_button(self, button: ButtonElement):
        if not (button is None or isinstance(button, ButtonElement)):
            isinstance(button, ButtonElement)
            raise AssertionError
    ...
    # def set_shift_button(self, button: ButtonElement):
    #   if not(button is None or isinstance(button, ButtonElement) and button.is_momentary()):
    #       isinstance(button, ButtonElement)
    #       raise AssertionError
    #   if self._shift_button is not button and self._shift_button is Not None:
    #       self._shift_button.remove_value_listener(self._shift_value)
    ...
    def _detail_view_visibility_changed(self):
        if self.is_enabled() and self._detail_toggle_button is not None:  #not self._shift_pressed and self._detail_toggle_button != None:
    ...
    def _device_clip_toggle_value(self, value):
        if self._device_clip_toggle_button is None:
            raise AssertionError
